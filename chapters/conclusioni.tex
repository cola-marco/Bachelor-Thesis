...
Il progetto ha portato alla realizzazione di una piattaforma completa di osservabilità per la \emph{web application PetClinic}, comprendente la raccolta e l'analisi di log, metriche, tracce distribuite e test sintetici. È stata inoltre configurata un'infrastruttura basata su \emph{Docker Compose}, integrata con \emph{Fleet}, \emph{Logstash} e l'\emph{OpenTelemetry Collector}, affiancata da strumenti di analisi avanzati quali \emph{Machine Learning} e \emph{Anomaly Detection}. \\
...
Al termine di questi test, la piattaforma è stata replicata in un ambiente di staging aziendale, per valutarne il funzionamento in un contesto reale. \\
...
RDF-11 & Il sistema dovrebbe consentire l'esportazione delle \emph{dashboard} o dei risultati delle \emph{query} in formato \gls{pdf}\glsfirstoccur o \gls{csv}\glsfirstoccur. & Desiderabile & Soddisfatto \\
\hline

RZF-12 & Il sistema può prevedere un modulo aggiuntivo per la generazione automatica di report periodici delle metriche raccolte. & Opzionale & Non applicabile \\
\hline

RZF-13 & Il sistema può consentire l'importazione automatica delle configurazioni \gls{apm} da ambienti di \emph{test} o \emph{staging}. & Opzionale & Non applicabile \\
\hline
...
RZQ-5 & Il sistema può includere un \emph{layout dark/light mode} o temi grafici personalizzati per una migliore leggibilità. & Opzionale & Non applicabile \\
\hline
...
ROV-3 & Tutti i componenti software devono essere compatibili con la versione di Linux installata (es. Ubuntu 22.04 LTS o Red Hat 9). & Obbligatorio & Soddisfatto \\
\hline
...
ROV-5 & La web application deve essere compatibile con i principali browser (Chrome >= 120, Firefox >= 115, Edge >= 120, Safari >= 15). & Obbligatorio & Soddisfatto \\
\hline

ROV-6 & Le configurazioni devono essere eseguite in ambiente Docker o su infrastruttura aziendale. & Obbligatorio & Soddisfatto \\
\hline

RDV-7 & La documentazione tecnica deve essere redatta in Markdown, LaTeX o PDF. & Desiderabile & Soddisfatto \\
\hline

RDV-8 & Il sistema dovrebbe supportare la distribuzione tramite Docker Compose. & Desiderabile & Soddisfatto \\
\hline
...
\begin{itemize}
    \item 8 requisiti funzionali obbligatori, 3 desiderabili e 2 opzionali;
    \item 3 requisiti non funzionali obbligatori e 2 desiderabili;
...
\end{itemize}
Complessivamente, il sistema realizzato rispetta pienamente le richieste principali definite nella fase di analisi dei requisiti, confermando la coerenza tra pianificazione progettuale e implementazione finale.
...
Lo sviluppo del progetto mi ha permesso di sviluppare competenze avanzate nell'ambito dell'osservabilità delle applicazioni web, comprendendo l'utilizzo di tecnologie quali \emph{Elasticsearch}, \emph{Kibana}, \emph{Fleet}, \emph{Logstash} e l'\emph{OpenTelemetry Collector}. \\
