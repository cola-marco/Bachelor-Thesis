...
\begin{itemize}
    ...
    \item \textbf{Integrazione tra componenti Elastic:}  
        il rischio si è verificato, nello specifico sono stati riscontrati alcuni problemi di comunicazione tra \emph{Logstash}, \emph{APM Server} e \emph{Elasticsearch}, ma sono stati risolti tramite \emph{debug} e lettura dei log.
    ...
    \item \textbf{Scalabilità e carico del sistema:}  
        il rischio si è manifestato solamente all'interno dell'ambiente locale \gls{wslg}\glsfirstoccur \emph{Linux} a causa di una impostazione errata di memoria al container di \emph{Elasticsearch}, ma è stato risolto aumentando la memoria allocata.

    \item \textbf{Implementazione di algoritmi di Machine Learning:}  
        alcuni limiti di licenza hanno inizialmente rallentato l'adozione del modulo \gls{mlg}, successivamente superati tramite l'attivazione del periodo di prova per i test locali.
...
ROV-5 & La \emph{web application} deve essere compatibile con i principali \emph{browser} (Chrome >= 120, Firefox >= 115, Edge >= 120, Safari >= 15). & Obbligatorio & Soddisfatto \\
\hline
...
RDV-7 & La documentazione tecnica deve essere redatta in \emph{Markdown}, \emph{LaTeX} o \emph{PDF}. & Desiderabile & Soddisfatto \\
\hline
