...
\begin{center}
\begin{longtable}{|p{1.3cm}|p{8.2cm}|p{2.5cm}|}
...
ROF-4     & Il sistema deve prevedere la configurazione di \emph{Elastic Agents} (\emph{Beats}/\emph{\gls{apmg}}) per la raccolta dati della navigazione. & Obbligatorio \\
\hline
...
RDF-11     & Il sistema dovrebbe consentire l'esportazione delle \emph{dashboard} o dei risultati delle \emph{query} in formato \gls{pdfg}\glsfirstoccur o \gls{csvg}\glsfirstoccur. & Desiderabile \\
\hline


RZF-12     & Il sistema può prevedere un modulo aggiuntivo per la generazione automatica di \emph{report} periodici delle metriche raccolte. & Opzionale \\
\hline


RZF-13     & Il sistema può consentire l'importazione automatica delle configurazioni \gls{apmg} da ambienti di \emph{test} o \emph{staging}. & Opzionale \\
\hline
...
compatibilità, strumenti, \emph{standard} aziendali o legali.
...
ROV-1 & Il sistema deve utilizzare i prodotti della \emph{suite} \emph{Elastic Stack}
(\emph{Elasticsearch, Logstash, Kibana, Beats, APM Server/Agent}). & Obbligatorio \\
\hline

ROV-2 & L'ambiente operativo deve essere \emph{Linux} (\emph{Red Hat} o distribuzioni equivalenti). & Obbligatorio \\
\hline

ROV-3 & Tutti i componenti \emph{software} devono essere compatibili con la versione di \emph{Linux} installata
(es. \emph{Ubuntu 22.04 LTS} o \emph{Red Hat 9}). & Obbligatorio \\
\hline
...
ROV-5 & La \emph{web application} deve essere compatibile con i principali \emph{browser}
(\emph{Chrome} >= 120, \emph{Firefox} >= 115, \emph{Edge} >= 120, \emph{Safari} >= 15). & Obbligatorio \\
\hline

ROV-6 & Le configurazioni devono essere eseguite in ambiente \emph{Docker} o su infrastruttura aziendale. & Obbligatorio \\
\hline

RDV-7 & La documentazione tecnica deve essere redatta in \emph{Markdown}, \emph{LaTeX} o \gls{pdfg}. & Desiderabile \\
\hline

RDV-8 & Il sistema dovrebbe supportare la distribuzione tramite \emph{Docker Compose}. & Desiderabile \\
\hline
...
La rappresentazione architetturale descrive la struttura della soluzione proposta, evidenziando le principali componenti coinvolte nel sistema di \gls{apmg} e le loro relazioni. \\
Essa costituisce il collegamento tra la fase di analisi dei requisiti e la successiva implementazione pratica. I dettagli tecnici e i diagrammi della soluzione verranno approfonditi nel capitolo dedicato allo sviluppo della piattaforma.


\subsection{Struttura generale della soluzione di monitoraggio}
La soluzione di \gls{apmg} proposta si fonda su un'architettura ibrida che combina le funzionalità di \emph{OpenTelemetry} con la potenza analitica dello \emph{Stack Elastic}, fornendo un sistema di osservabilità completo. \\
L'applicazione di riferimento, \emph{PetClinic}, viene monitorata sul lato \emph{backend} tramite l'\emph{OpenTelemetry Java Agent}, avviato insieme all'applicazione mediante parametri \texttt{-Dotel}. L'agente intercetta automaticamente chiamate \gls{httpg}\glsfirstoccur, operazioni sul \emph{database} e transazioni applicative, generando le relative tracce e metriche. \\
A differenza di un'integrazione diretta, i dati raccolti non vengono inviati immediatamente all'\emph{APM Server}, ma passano prima attraverso l'\emph{OpenTelemetry Collector}. Il \emph{Collector} funge da livello intermedio dedicato all'elaborazione e arricchimento dei dati di telemetria, oltre a gestire la traduzione del formato in modo efficiente. \\
Una volta processati, i dati vengono inoltrati all'\emph{APM Server} (integrato nell'\emph{Elastic Agent} gestito tramite \emph{Fleet}) utilizzando il protocollo \gls{otlpg}\glsfirstoccur. \\
L'\emph{APM Server} riceve i dati normalizzati dal \emph{Collector}, li converte nel formato interno di \emph{Elastic} e li invia a \emph{Elasticsearch} per l'indicizzazione. \\
Parallelamente, il monitoraggio del \emph{frontend} è gestito dal \emph{RUM Agent JavaScript}, che traccia interazioni utente, tempi di caricamento, prestazioni percepite nel \emph{browser} ed eventuali errori \emph{JavaScript}.
Il \emph{RUM Agent} comunica direttamente con l'\emph{APM Server}, senza passare dal \emph{Collector}. \\
Tutti i dati prodotti dai vari agenti convergono infine in \emph{Elasticsearch}, dove vengono memorizzati e resi disponibili alla consultazione tramite \emph{Kibana}.
...
\begin{itemize}
    ...
    \item \textbf{APM Server:} incluso all'interno dell'\emph{Elastic Agent}, riceve i dati di telemetria provenienti dagli agenti applicativi. Supporta lo standard \emph{OpenTelemetry}, fungendo da \emph{endpoint} compatibile per la ricezione dei dati raccolti dal \emph{Java Agent};
    
    \item \textbf{OpenTelemetry Collector:} componente intermedio che riceve i dati di telemetria dall'\emph{OpenTelemetry Java Agent}, li elabora e li inoltra all'\emph{APM Server} tramite \gls{otlpg};

    \item \textbf{OpenTelemetry Java Agent:} agente di strumentazione automatica avviato con l'applicazione \emph{PetClinic}, utilizzato per raccogliere metriche e tracce delle transazioni lato \emph{backend} (chiamate \gls{httpg}, query al \emph{database}, eccezioni, tempi di risposta). I dati generati vengono inviati all'\emph{APM Server} attraverso il protocollo \gls{otlpg};
    
    \item \textbf{RUM Agent JavaScript:} agente di monitoraggio \gls{rumg}\glsfirstoccur eseguito nel \emph{frontend} dell'applicazione, che raccoglie dati sulle interazioni utente, sui tempi di caricamento delle pagine e sulle prestazioni percepite nel \emph{browser};
...
\begin{figure}[!h]
    ...
    \caption{Integrazione tra componenti del sistema di monitoraggio}
\end{figure}
...
Tale struttura consente di osservare comportamenti applicativi realistici, come chiamate \gls{httpg}, interazioni con il \emph{database} e logiche di \emph{business}, fornendo un contesto ideale per testare le funzionalità di monitoraggio offerte da \emph{Elastic APM} e \emph{OpenTelemetry}. \\

\begin{figure}[!h] 
    \centering 
    \includegraphics[width=\columnwidth]{Architettura_PetClinic.png} 
    \caption{Architettura di Spring PetClinic}
\end{figure}
...
Le seguenti \emph{user stories} descrivono brevemente i principali scenari di utilizzo del sistema di \gls{apmg}, evidenziando gli obiettivi e le esigenze degli utenti tecnici aziendali che operano sulla piattaforma di monitoraggio. \\
...
\textbf{US2} & Come \emph{Observability Engineer}, voglio monitorare l'utilizzo di \gls{cpug}\glsfirstoccur, memoria e connessioni per valutare la stabilità del sistema. & Garantire la corretta gestione delle risorse. \\
\hline
...
\textbf{US10} & Come \emph{Observability Engineer}, voglio integrare l'\emph{OpenTelemetry Java Agent} e il \emph{RUM Agent JavaScript} per raccogliere rispettivamente metriche di \emph{backend} e \emph{frontend}. & Avere una visione completa del comportamento dell'applicazione. \\
\hline
...
\textbf{US13} & Come \emph{Observability Engineer}, voglio esportare le \emph{dashboard} in formato \gls{pdfg} o \gls{csvg} per la condivisione all'interno del \emph{team}. & Documentare e condividere le analisi in modo efficace. \\
\hline

\textbf{US14} & Come \emph{Observability Engineer}, voglio ricevere notifiche automatiche via \emph{email} quando vengono superate determinate soglie di prestazione. & Automatizzare la gestione degli incidenti e migliorare il tempo di reazione. \\
\hline
