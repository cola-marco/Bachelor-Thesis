...
ROV-3 & Tutti i componenti software devono essere compatibili con la versione di Linux installata
(es. Ubuntu 22.04 LTS o Red Hat 9). & Obbligatorio \\
\hline
...
\begin{itemize}
    ...
    \item Kibana >= 8.10
    \item Beats >= 8.10
    \item APM Server >= 8.10
    \item Elasticsearch >= 8.10
    \item OpenTelemetry >= 1.0
\end{itemize} & Obbligatorio \\
\hline

ROV-5 & La web application deve essere compatibile con i principali browser
(Chrome >= 120, Firefox >= 115, Edge >= 120, Safari >= 15). & Obbligatorio \\
\hline

ROV-6 & Le configurazioni devono essere eseguite in ambiente Docker o su infrastruttura aziendale. & Obbligatorio \\
\hline

RDV-7 & La documentazione tecnica deve essere redatta in Markdown, LaTeX o PDF. & Desiderabile \\
\hline

RDV-8 & Il sistema dovrebbe supportare la distribuzione tramite Docker Compose. & Desiderabile \\
\hline
...
La soluzione di \gls{apm} proposta si fonda su un'architettura ibrida che combina le funzionalità di \emph{OpenTelemetry} con la potenza analitica dello \emph{Stack Elastic}, fornendo un sistema di osservabilità completo. \\
L'applicazione di riferimento, \emph{PetClinic}, viene monitorata sul lato \emph{backend} tramite l'\emph{OpenTelemetry Java Agent}, avviato insieme all'applicazione mediante parametri \texttt{-Dotel}. L'agente intercetta automaticamente chiamate \gls{httpg}, operazioni sul database e transazioni applicative, generando le relative tracce e metriche. \\
...
Una volta processati, i dati vengono inoltrati all'\emph{APM Server} (integrato nell'\emph{Elastic Agent} gestito tramite \emph{Fleet}) utilizzando il protocollo \gls{otlpg}. \\
