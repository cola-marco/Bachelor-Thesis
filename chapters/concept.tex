...
L'applicazione di riferimento \emph{PetClinic}, viene monitorata tramite l'\emph{OpenTelemetry Java Agent}, avviato contestualmente all'applicazione mediante comandi \emph{-Dotel} nello script di avvio. Questo agente intercetta automaticamente le chiamate \gls{httpg}, le transazioni e le operazioni sul database, generando metriche e tracce relative al comportamento del \emph{backend}. \\
I dati raccolti vengono inviati all'\emph{APM Server}, gestito da \emph{Fleet} all'interno dell'\emph{Elastic Agent}. L'\emph{APM Server} riceve, elabora e normalizza le informazioni provenienti dall'agente \emph{OpenTelemetry}, rendendole compatibili con \emph{Elasticsearch}. \\
In contemporanea, il monitoraggio del \emph{frontend} è gestito dal \emph{RUM Agent JavaScript}, che traccia le interazioni dell'utente, i tempi di caricamento delle pagine e gli eventi di errore nel \emph{browser}. \\
Tutti i dati vengono centralizzati in \emph{Elasticsearch}, dove sono indicizzati e resi disponibili per la consultazione tramite \emph{Kibana}.
...
\begin{figure}[!h] 
    \centering 
    \includegraphics[width=\textwidth]{Integrazione_APM-Open.png} 
